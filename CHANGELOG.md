...
- Continuous dev releases: every merge to develop publishes a pre-release and updates the `hush` cask in ryan-stoffel/homebrew-taps, so `brew upgrade hush` installs the latest build. Signing accepts any persistent certificate so permission grants survive upgrades. (#119)
