...
Both channels update the Homebrew cask `hush` in `ryan-stoffel/homebrew-taps` when the `TAP_TOKEN` secret is configured, so `brew upgrade hush` moves an installed copy to the newest build of either channel. See [Dev channel and Homebrew](#dev-channel-and-homebrew).
...
3. Detects optional secrets. `HAS_SIGNING` is true when `SIGNING_CERT_P12_BASE64` is non-empty. `HAS_NOTARY` is true when both that and `NOTARY_PASSWORD` are non-empty. `HAS_SPARKLE_KEY` and `HAS_TAP_TOKEN` follow `SPARKLE_ED_PRIVATE_KEY` and `TAP_TOKEN`.
...
12. If `HAS_TAP_TOKEN` is true: writes `Casks/hush.rb` in `ryan-stoffel/homebrew-taps` with the new version, SHA-256, and download URL (`scripts/update_cask.py`) and pushes it. Otherwise a notice says the cask was not updated.
...
`GITHUB_TOKEN` cannot write to another repository, so the cask update needs a token that can. Create a fine-grained personal access token at <https://github.com/settings/personal-access-tokens/new> with:
...
```sh
gh secret set TAP_TOKEN --repo ryan-stoffel/hush
```

Without the secret every other step still runs and the release is still published; only the cask stays at its previous version.
...
| `TAP_TOKEN` | Fine-grained personal access token with Contents read and write on `ryan-stoffel/homebrew-taps`. | See [The TAP_TOKEN secret](#the-tap_token-secret). |
...
When `TAP_TOKEN` is empty: the cask step is skipped with the notice `TAP_TOKEN is not configured. The Homebrew cask in ryan-stoffel/homebrew-taps was not updated.`
