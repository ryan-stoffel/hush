...
Both channels update the Homebrew cask `hush` in `ryan-stoffel/homebrew-taps` when the `TAP_DEPLOY_KEY` (or `TAP_TOKEN`) secret is configured, so `brew upgrade --cask ryan-stoffel/taps/hush` moves an installed copy to the newest build of either channel. See [Dev channel and Homebrew](#dev-channel-and-homebrew).
...
The fully qualified name is required for both commands. homebrew-cask ships an unrelated `hush` (a Safari extension), and Homebrew's cask loader checks homebrew-cask before any third-party tap even when the tap's cask is the installed one, so `brew install --cask hush` or `brew upgrade --cask hush` installs that extension over Hush.app. Plain `brew upgrade` with no name is safe: it upgrades each installed cask from the tap recorded at install time. The cask's caveats repeat this.
