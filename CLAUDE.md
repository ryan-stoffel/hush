...
CLAUDE.md and AGENTS.md carry the same content and must be kept in sync. Any change to this file must be made to AGENTS.md in the same commit. This file has one extra final section, "Claude Code notes", that AGENTS.md does not have.
...
- Use the `gh` CLI for issues and PRs (`gh issue view`, `gh issue create`, `gh pr create --base develop`, `gh pr checks`).
- Never add Claude or Anthropic attribution, co-author trailers, or "generated with" lines to commits or PR bodies.
- Never push to `main` or `develop`, and never force push.
- Ask before any destructive git command (`reset --hard`, `clean`, `checkout --` over local changes, branch deletion, history rewrites).
- The generated `Quoth.xcodeproj` is disposable. Change `project.yml` and regenerate with `xcodegen generate` instead of editing the project.
- UI tests take over the screen. Tell the user before running `scripts/test.sh ui` (or plain `scripts/test.sh`, or `scripts/capture-screenshots.sh`) locally.
- CI rewrites the PR body between the screenshots markers. Do not overwrite it with `gh pr edit --body` from a stale copy. Fetch the current body first (`gh pr view <number> --json body -q .body`), edit outside the marker block, and keep the marker block exactly as CI wrote it.
- Prefer `scripts/` over ad hoc `xcodebuild` invocations. The scripts set the scheme, DerivedData path, result bundle path, and ad hoc signing the same way CI does.
